...
pub fn main() anyerror!void {
                ...
                for (i..pages_arr.items.len) |j| {
                    for (instr_arr.items) |cur_instr| {
                        if (!cur_instr.check_if_ok(pages_arr.items[i], pages_arr.items[j])) {
                            ok = false;
                            const temp = pages_arr.items[i];
                            pages_arr.items[i] = pages_arr.items[j];
                            pages_arr.items[j] = temp;
                            continue :start_over;
                        }
                    }
                }
    ...
    print("Total corrected print updates correct middle elements sum: {d}", .{counter});
}
